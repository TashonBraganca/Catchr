...
export const storageClient = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
...
export class StorageManager {
     ...
     */
    static async validateUpload(userId, fileSize, mimeType) {
        try {
            // Check file size
     ...
     */
    static async cleanupOldFiles(userId, retentionDays) {
        try {
     ...
     */
    static async getDownloadUrl(filePath, expiresIn = 3600 // 1 hour default
    ) {
        try {
     ...
     */
    static async moveFile(fromPath, toPath, fromBucket = STORAGE_CONFIG.buckets.AUDIO_CAPTURES, toBucket = STORAGE_CONFIG.buckets.AUDIO_CAPTURES) {
        try {
     ...
     * Batch delete files
     */
    static async batchDelete(filePaths, bucket = STORAGE_CONFIG.buckets.AUDIO_CAPTURES) {
