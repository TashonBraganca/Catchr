...
import { queueService, JobType } from '../services/queueService.js';
// Initialize services
...
export class AIWorker {
    ...
    setupWorkers() {
        ...
        const whisperWorker = new Worker(JobType.WHISPER_TRANSCRIBE, async (job) => {
        ...
        const enrichWorker = new Worker(JobType.ENRICH_SUMMARY, async (job) => {
        ...
        const calendarWorker = new Worker(JobType.CALENDAR_CREATE, async (job) => {
    ...
    async processWhisperTranscription(job) {
        ...
        try {
            ...
            // Update processing status
            await this.updateProcessingStatus(thoughtId, 'failed', 'whisper_transcription', error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    async processEnrichment(job) {
        ...
        try {
            ...
            // Update processing status
            await this.updateProcessingStatus(thoughtId, 'failed', 'categorization', error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    async processCalendarCreation(job) {
        ...
        try {
            ...
            await this.updateProcessingStatus(thoughtId, 'failed', 'calendar_integration', error instanceof Error ? error.message : 'Unknown error');
            throw error;
        }
    }
    ...
    async updateProcessingStatus(thoughtId, status, processingType, errorMessage) {
        try {
            const updateData = {
                status,
                updated_at: new Date().toISOString(),
    ...
    async sendRealtimeNotification(userId, eventType, payload) {
        try {
