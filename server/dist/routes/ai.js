...
router.post('/categorize', authenticateToken, async (req, res) => {
    try {
        ...
        const categorization = await ultrathinkAI.categorizeThought(content, userId, enhancedContext);
...
router.post('/enhance', authenticateToken, async (req, res) => {
    try {
        ...
        const categorization = await ultrathinkAI.categorizeThought(content, userId, {
            currentCategory,
            userFeedback,
...
router.post('/batch-categorize', authenticateToken, async (req, res) => {
    try {
        ...
        const results = await Promise.allSettled(thoughts.map((content, index) => ultrathinkAI.categorizeThought(content, userId, { batchIndex: index })));
