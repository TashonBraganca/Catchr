...
router.post('/sync',
  ...
  async (req, res) => {
    try {
      ...
      try {
        decodedToken = jwt.verify(token, process.env.JWT_SECRET || 'dev-secret') as any;
      } catch (error) {
        return res.status(401).json({
          success: false,
...
router.post('/disconnect',
  ...
  async (req, res) => {
    try {
      ...
      const token = authHeader.slice(7);
      const decodedToken = jwt.verify(token, process.env.JWT_SECRET || 'dev-secret') as any;
...
async function triggerCaptureWebhooks(capture: any): Promise<void> {
  try {
