...
import { TranscriptionRequest, TranscriptionResponse } from '@cathcr/shared';
...
export class HuggingFaceService {
  ...
  async transcribeAudioFromFile(filePath: string, options?: {
    model?: string;
    language?: string;
  }): Promise<TranscriptionResponse> {
    try {
   ...
   */
  async batchTranscribe(audioBuffers: Buffer[], options?: {
    model?: string;
    language?: string;
  }): Promise<TranscriptionResponse[]> {
    try {
      ...
      for (let i = 0; i < audioBuffers.length; i += batchSize) {
        ...
        const batchResults = await Promise.all(
          batch.map(buffer => this.transcribeAudio(buffer, options))
        );
   ...
   */
  getModelInfo(modelName: string): { name: string; size: string; languages: string[] } | null {
    const modelMap: { [key: string]: { name: string; size: string; languages: string[] } } = {
      'openai/whisper-large-v3': {
