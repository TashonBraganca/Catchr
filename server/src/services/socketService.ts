...
export const setupSocketHandlers = (io: SocketIOServer): void => {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('authenticate', ({ userId, username }: { userId: string; username: string }) => {
      ...
      const userRoomsData: ChatRoom[] = Array.from(userRoomIds)
        .map(roomId => {
    ...
    socket.on('create_room', ({ name, creatorId }: { name: string; creatorId: string }) => {
      try {
        const roomId = uuidv4();
        const room: RoomData = {
          id: roomId,
    ...
    socket.on('send_message', (messageData: Omit<Message, 'id' | 'createdAt'>) => {
      const user = connectedUsers.get(socket.id);
