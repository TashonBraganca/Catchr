...
export class TranscriptionService {
  ...
  private fasterWhisperEnabled: boolean = false;
  ...
  private getHuggingFaceService(): HuggingFaceService | null {
    return this.huggingFaceService;
  }

  private async checkFasterWhisperAvailability(): Promise<void> {
    try {
  ...
  async transcribeAudio(request: TranscriptionRequest): Promise<TranscriptionResult> {
    ...
    if (request.webSpeechText) {
      ...
      if (request.audioBuffer || request.audioUrl) {
        try {
          const enhancedResult = await this.enhanceWebSpeechTranscription(
            request.webSpeechText,
            request.audioBuffer,
            request.audioUrl
          );
          if (enhancedResult) {
   ...
   */
  private async transcribeServerSide(request: TranscriptionRequest): Promise<TranscriptionResult> {
    const startTime = Date.now();
   ...
   */
  private async enhanceWebSpeechTranscription(
    webSpeechText: string,
    audioBuffer?: Buffer,
    audioUrl?: string
  ...
  private compareTranscriptions(
    webSpeechText: string,
    serverResult: TranscriptionResult
  ): TranscriptionResult {
  ...
  private async transcribeWithFasterWhisper(
    audioBuffer: Buffer,
    language?: string
  ): Promise<{ text: string; confidence: number; word_timestamps?: any[] }> {
  ...
  async batchTranscribe(
    ...
    for (let i = 0; i < requests.length; i += batchSize) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map(request => this.transcribeAudio(request))
      );
