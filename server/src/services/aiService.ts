...
export class AIService {
  ...
  async categorizeThought(request: AICategorizationRequest): Promise<AICategorizationResponse> {
    try {
  ...
  async extractReminders(thought: string): Promise<ReminderInfo | null> {
    try {
  ...
  async transcribeAudio(audioBuffer: Buffer, filename = 'audio.webm'): Promise<{ text: string; confidence: number }> {
    try {
  ...
  async transcribeAudioFromFile(filePath: string): Promise<{ text: string; confidence: number }> {
    try {
  ...
  async suggestExpansions(thought: string, category: ThoughtCategory): Promise<string[]> {
    try {
      const prompt = `
...
Be concise but thorough in your analysis.`;
  ...
  private buildCategorizationContext(userPreferences: UserPreferences, previousThoughts?: any[]): string {
  ...
  private buildCategorizationPrompt(thought: string, context: string): string {
    return `${context}
...
Consider the user's preferences and recent thought patterns. Provide detailed categorization with high confidence scores.`;
  ...
  private async processAIResponse(aiResult: any, originalThought: string): Promise<AICategorizationResponse> {
    // Validate and clean the AI response
    const category: ThoughtCategory = {
    ...
    const entities: ExtractedEntity[] = (aiResult.entities || []).map((entity: any) => ({
    ...
    let reminder: ReminderInfo | undefined;
  ...
  private fallbackCategorization(thought: string): AICategorizationResponse {
    // Simple rule-based categorization as fallback
    const lowerThought = thought.toLowerCase();
    
    let category: ThoughtCategory = { main: 'notes', color: '#6B7280', icon: '📝' };
    const tags: string[] = [];
  ...
  async batchProcessThoughts(thoughtIds: string[], userId: string): Promise<void> {
    try {
